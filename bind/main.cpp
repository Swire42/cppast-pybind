...
struct Binder {
  ...
  void print(std::ostream& out) {
    ...
    out << "PYBIND11_MODULE(" << name << ", m) {\n";

    for (Def const& def : functions) {
      out << "  m" << def.str() << ";\n";
    }
...
Def process_function(cppast::cpp_function const& func)
{
  return Def(func.name());
}

// prints the AST of a file
void process_file(std::ostream& out, const cppast::cpp_file& file)
{
  Binder binder("example");

  binder.includes.push_back(file.name());

  for (cppast::cpp_entity const& entity : file) {
    if (entity.kind() == cppast::cpp_entity_kind::function_t) {
      binder.functions.push_back(process_function(dynamic_cast<cppast::cpp_function const&>(entity)));
    }
  }
